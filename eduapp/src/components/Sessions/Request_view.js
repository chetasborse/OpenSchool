...
class Request_view extends Component {
  ...
  componentDidMount() {
    if (this.props.verfied === 1) {
      axios
        .get("http://localhost:5000/session/request", {
          params: {
      ...
      .then((response) => {
        ...
        this.setState((state) => {
          const requests = state.requests.filter(
            (req1) => req1.request_id !== req.request_id
          );
          return {
            requests,
          };
      ...
      <React.Fragment key={req.request_id}>
        <Container style={{ border: "1px solid black" }}>
          ...
          <Row>
            ...
            <Col sm={3}>Grade: {req.grade}</Col>
            <Col sm={3}>Board: {req.board}</Col>
          </Row>
          <Row>
            <Col sm={6}>
              Subject:{" "}
          ...
          <Row>
            <Col sm={6}>Time Slot: {req.time_slot}</Col>
            <Col sm={3}>Date: {req.req_date}</Col>
          </Row>
          <Container style={{ textAlign: "center" }}>
            <Button color="success" onClick={() => this.approveRequest(req)}>
              Respond
            </Button>
          </Container>
        </Container>
      ...
      <div className="toplookout">
        {this.state.verfied == 1 ? (
          <React.Fragment>
            ...
            <Container style={{ border: "1px solid #cecece" }}>
              {this.state.requests.length === 0 ? (
                <Container>No requests</Container>
              ) : (
          ...
          <React.Fragment>
            ...
            <p>
              You will see the requests once the administrator verifies your
              profile
            </p>
            <p>
              If you haven't mailed your qualification documnents yet, please do
              it at the earliest
            </p>
          </React.Fragment>
        ) : (
          <React.Fragment>
            <h3>
              Sorry {this.props.first_name} {this.props.last_name}, your account
              has been <span style={{ color: "red" }}>suspended</span>
            </h3>
          </React.Fragment>
        )}
      </div>
